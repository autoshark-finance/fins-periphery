...
import IFinsV2Pair from '../build/IFinsPair.json'
...
describe('FinsV2Router02', () => {
  ...
  it('getAmountOut', async () => {
    expect(await router.getAmountOut(bigNumberify(2), bigNumberify(100), bigNumberify(100), swapFee)).to.eq(bigNumberify(1))
    await expect(router.getAmountOut(bigNumberify(0), bigNumberify(100), bigNumberify(100), swapFee)).to.be.revertedWith(
      'FinsV2Library: INSUFFICIENT_INPUT_AMOUNT'
    )
    await expect(router.getAmountOut(bigNumberify(2), bigNumberify(0), bigNumberify(100), swapFee)).to.be.revertedWith(
      'FinsV2Library: INSUFFICIENT_LIQUIDITY'
    )
    await expect(router.getAmountOut(bigNumberify(2), bigNumberify(100), bigNumberify(0), swapFee)).to.be.revertedWith(
      'FinsV2Library: INSUFFICIENT_LIQUIDITY'
    )
  })

  it('getAmountIn', async () => {
    expect(await router.getAmountIn(bigNumberify(1), bigNumberify(100), bigNumberify(100), swapFee)).to.eq(bigNumberify(2))
    await expect(router.getAmountIn(bigNumberify(0), bigNumberify(100), bigNumberify(100), swapFee)).to.be.revertedWith(
      'FinsV2Library: INSUFFICIENT_OUTPUT_AMOUNT'
    )
    await expect(router.getAmountIn(bigNumberify(1), bigNumberify(0), bigNumberify(100), swapFee)).to.be.revertedWith(
      'FinsV2Library: INSUFFICIENT_LIQUIDITY'
    )
    await expect(router.getAmountIn(bigNumberify(1), bigNumberify(100), bigNumberify(0), swapFee)).to.be.revertedWith(
      'FinsV2Library: INSUFFICIENT_LIQUIDITY'
    )
  })
...
describe('fee-on-transfer tokens', () => {
  ...
  beforeEach(async function() {
    ...
    pair = new Contract(pairAddress, JSON.stringify(IFinsV2Pair.abi), provider).connect(wallet)
  })
